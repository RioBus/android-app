...
public class RecebeDadosOnibusTask extends AsyncTask<String,Void, List<Bus>> {
    ...
    protected List<Bus> doInBackground(String... params) {
        ...
        parametros.add(new BasicNameValuePair("platformId", "2"));
        parametros.add(new BasicNameValuePair("lines", linha));
    ...
    protected void onPostExecute(List<Bus> buses) {
        ...
        iRecebeDadosOnibus.recebeListaPontosCallback(buses, erro);

    }

    // criar classe json parser
    private List<Bus> getPonto(BufferedReader reader) {
        Gson gson = new Gson();
        JsonParser parser = new JsonParser();
        List<Bus> buses = new ArrayList<Bus>();
        ...
        for (JsonElement elem: data) {
            ...
            Bus bus = new Bus();
            bus.setTimestamp(converteStringToDate(getJsonAsString(obj.get(0))));
            bus.setOrder(getJsonAsString(obj.get(1)));
            bus.setLine(getJsonAsString(obj.get(2)));
            bus.setLatitude(getJsonAsDouble(obj.get(3)));
            bus.setLongitude(getJsonAsDouble(obj.get(4)));
            bus.setVelocity(getJsonAsDouble(obj.get(5)));
            buses.add(bus);
        }
        return buses;
    }
